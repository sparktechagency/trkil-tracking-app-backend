...
const createWorkToDB = async (payload: IRule) => {

    const isExistWork = await Rule.findOne({ type: 'work' })
    if (isExistWork) {
        const result = await Rule.findOneAndUpdate({type: 'work'}, {content: payload?.content}, {new: true})
        const message = "Work Functionality Updated successfully"
        return { message, result }
    } else {
        const result = await Rule.create({ ...payload, type: 'work' })
        const message = "Work Functionality created successfully"
        return { message, result }
    }
}
  
const getWorkFromDB = async () => {

    const result = await Rule.findOne({ type: 'work' })
    if (!result) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Work Functionality doesn't exist!")
    }
    return result
}
