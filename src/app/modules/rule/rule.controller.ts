...
const createWork = catchAsync(async (req: Request, res: Response) => {
    const { ...aboutData } = req.body
    const result = await RuleService.createWorkToDB(aboutData)
  
    sendResponse(res, {
        success: true,
        statusCode: StatusCodes.OK,
        message: 'Work Functionality created successfully',
        data: result
    })
})
  
const getWork = catchAsync(async (req: Request, res: Response) => {
    const result = await RuleService.getWorkFromDB()
  
    sendResponse(res, {
        success: true,
        statusCode: StatusCodes.OK,
        message: 'Work Functionality retrieved successfully',
        data: result
    })
})
